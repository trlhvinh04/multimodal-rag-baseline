...
def prepare_df_for_prompt(df1, df2, n_rows=7):
    def get_input_df(df):
        ...
        header = not (
            bool(unnamed_cols) or numeric_cols
        )  # Don't show header if unnamed or numeric columns
...
def generate(prompt):
    ...
    generate_content_config = types.GenerateContentConfig(
        ...
        response_schema=genai.types.Schema(
            ...
            properties={
                "is_continuation": genai.types.Schema(
        ...
        system_instruction=[
            types.Part.from_text(
                text="""You are skilled at analyzing fragmented tables. Given two table fragments (`table_fragment_1`, `table_fragment_2`) in plain text (CSV) but the separator is \"=\" perform the following:
...
2. If not a continuation:
    ...
    res = handle_response(response.text)
    
    # Print output token count 
    print(f"Output tokens: {len(response.text.split())}")
